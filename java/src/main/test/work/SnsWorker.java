	...
	private class LoginReq implements Callable<Boolean> {
		...
		public Boolean call() {
			try {
				...
				byte[] resp = HttpUtils.httpPost(SnsWorker.this.getUrl(), loginReq.bytes());
	...
	private class EnterServerReq implements Callable<Boolean> {
		...
		public Boolean call() {
			try {
				...
				byte[] datas = HttpUtils.httpPost(SnsWorker.this.getUrl(), es.bytes());
